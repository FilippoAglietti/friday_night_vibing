import type { Translations } from "../types";

const fr: Translations = {
  nav: {
    howItWorks: "Comment ça marche",
    examples: "Exemples",
    pricing: "Tarifs",
    tutorial: "Tutorial",
    contactUs: "Nous contacter",
    getStartedFree: "Commencer gratuitement",
  },
  hero: {
    badge: "Cours audio · Liens partageables · Design époustouflant",
    title1: "Le Générateur de Cours",
    titleConnector: "qui",
    title2: "Sonne Aussi Bien Qu'il en a l'Air",
    subtitle:
      "L'IA crée des cours complets avec narration audio, un design soigné et des liens partageables — prêts à enseigner, vendre ou partager en quelques secondes.",
    cta: "Générez Votre Premier Cours Gratuitement",
    secondaryCta: "Écouter un cours exemple",
    tagline: "Sans carte bancaire · Votre premier cours est gratuit",
  },
  problem: {
    eyebrow: "Le Problème",
    heading: "Ça vous parle ?",
    subheading:
      "Les autres outils IA vous donnent un bloc de texte et appellent ça un cours. Vos apprenants méritent un audio à écouter, un design à regarder et un lien qu'ils peuvent vraiment ouvrir.",
    pain1Problem: "Vos cours ressemblent à ceux de tout le monde",
    pain1Solution:
      "Syllabi génère des cours avec un design époustouflant et une vraie narration audio — tellement soignés que vos apprenants n'en croiront pas que c'est de l'IA.",
    pain2Problem: "Personne ne termine vos cours",
    pain2Solution:
      "Les leçons audio, le rythme structuré et les visuels soignés maintiennent les apprenants engagés du début à la fin — pas juste en survol.",
    pain3Problem: "Créer et partager est un cauchemar à 10 outils",
    pain3Solution:
      "Un clic vous donne un cours complet avec un lien partageable, de l'audio, des quiz et des fichiers prêts à exporter. Aucun LMS requis.",
  },
  howItWorks: {
    eyebrow: "Comment ça marche",
    heading: "Trois étapes. Zéro prise de tête.",
    step1Title: "Décrivez ce que vous enseignez",
    step1Desc:
      'Entrez votre sujet, choisissez un style et un niveau — de « Python pour la science des données » à « Boulangerie au levain 101 ».',
    step2Title: "L'IA construit votre cours",
    step2Desc:
      "En quelques secondes, vous obtenez des modules, des leçons, des quiz, une narration audio et un lien partageable — tout est modifiable.",
    step3Title: "Partagez, enseignez ou vendez",
    step3Desc:
      "Envoyez un lien, exportez vers Notion ou PDF, ou transformez-le en lead magnet. Votre cours, à votre façon.",
  },
  generate: {
    eyebrow: "Essayez maintenant",
    headingPick: "Choisissez un point de départ",
    headingCustomize: "Personnalisez et générez",
    subheading: "Choisissez un modèle pour démarrer rapidement ou construisez de zéro.",
    backToTemplates: "Retour aux modèles",
  },
  templates: {
    onboardingLabel: "Onboarding des employés",
    onboardingDesc: "Formation des nouveaux collaborateurs sur la culture, les processus et les outils",
    leadMagnetLabel: "Mini-Cours Lead Magnet",
    leadMagnetDesc: "Cours à gain rapide pour capturer des e-mails et développer votre liste",
    coachingLabel: "Programme de coaching",
    coachingDesc: "Parcours de transformation structuré pour les clients",
    salesLabel: "Formation à la vente",
    salesDesc: "Frameworks de pitch, gestion des objections et techniques de closing",
    technicalLabel: "Compétences techniques",
    technicalDesc: "Programmation, outils ou workflows techniques — pratique",
    scratchLabel: "Partir de zéro",
    scratchDesc: "Contrôle total — vous définissez chaque détail",
  },
  examples: {
    eyebrow: "Exemples réels",
    heading: "Écoutez. Voyez. Partagez.",
    subheading:
      "Chaque cours ci-dessous a été généré en quelques secondes — avec narration audio, leçons structurées, quiz et lien partageable.",
    modules: "Modules",
    lessons: "Leçons",
    total: "Total",
    preview: "Aperçu",
  },
  testimonials: {
    eyebrow: "Ce que disent les créateurs",
    heading: "Ils ont tout essayé. Puis ils ont entendu Syllabi.ai",
    subheading:
      "Des créateurs de cours, coachs et formateurs qui sont passés aux cours axés sur l'audio et ne sont jamais revenus en arrière.",
  },
  pricing: {
    eyebrow: "Tarifs",
    heading: "Commencez gratuitement. Passez à la version supérieure quand vous êtes prêt.",
    launchSpecial: "Offre de lancement",
    endsIn: "Se termine dans",
    mostPopular: "Le plus populaire",
    oneTime: "Paiement unique",
    bestValue: "Meilleur rapport qualité-prix",
    forever: "/à vie",
    month: "/mois",
    oneTimeLabel: "paiement unique",
    getStartedFree: "Commencer gratuitement",
    startFree: "Commencer gratuitement",
    save20: "Économisez 20 %",
    save21: "Économisez 21 %",
    save13: "Économisez 13 %",
    freeDesc: "Créez 3 mini-cours gratuitement. Sans carte bancaire.",
    proDesc: "Pour les créateurs de cours sérieux qui publient régulièrement.",
    fivePackDesc: "5 générations premium. Sans abonnement.",
    proMaxDesc: "La boîte à outils ultime pour créer et vendre des cours.",
    aiAudioTitle: "Leçons audio IA",
    aiAudioDesc: "Générez une narration audio pour chaque leçon",
    swipePlans: "Faites glisser pour voir tous les forfaits →",
    startProBtn: "Passer à Pro — €28/mois",
    tryProMaxBtn: "Essayer Pro Max — €33",
    goProMaxBtn: "Passer à Pro Max — €69/mois",
    free1: "3 générations de mini-cours gratuites",
    free2: "Modules, leçons et quiz",
    free3: "Export PDF et Notion",
    free4: "Liens de cours partageables",
    free5: "Narration audio",
    free6: "15 générations/mois (Pro)",
    pro1: "15 générations de cours par mois",
    pro2: "Modules, leçons et quiz complets",
    pro3: "Export PDF, Markdown et Notion",
    pro4: "Tous les formats et styles de cours",
    pro5: "Traitement IA prioritaire",
    pm1: "Tout ce qu'il y a dans Pro",
    pm2: "Leçons audio générées par IA",
    pm3: "Génération complète du contenu par chapitre",
    pm4: "Export premium Notion et PDF",
    pm5: "Packages de cours prêts à vendre",
    pm6: "Marque white-label",
    pm7: "Traitement IA dédié",
    pack1: "5 générations Pro Max",
    pack2: "Leçons audio générées par IA",
    pack3: "Génération complète du contenu par chapitre",
    pack4: "Export premium Notion et PDF",
    pack5: "Marque white-label",
    pack6: "Aucun frais récurrents",
  },
  finalCta: {
    heading: "Prêt à entendre la différence ?",
    subheading:
      "Créez un cours avec audio, design et lien partageable — le tout en moins de 60 secondes. Votre première génération est gratuite.",
    cta: "Générez Votre Premier Cours Gratuitement",
  },
  footer: {
    tagline: "Génération de cours par IA pour les créateurs de cours, éducateurs et coachs.",
    product: "Produit",
    resources: "Ressources",
    legal: "Mentions légales",
    howItWorks: "Comment ça marche",
    examples: "Exemples",
    pricing: "Tarifs",
    tutorial: "Tutorial",
    api: "API",
    contactUs: "Nous contacter",
    documentation: "Documentation",
    blog: "Blog",
    changelog: "Changelog",
    support: "Support",
    feedback: "Feedback",
    privacyPolicy: "Politique de confidentialité",
    termsOfService: "Conditions d'utilisation",
    cookiePolicy: "Politique des cookies",
    copyright: "© {year} Syllabi. Tous droits réservés.",
    builtWith: "Fait avec ❤️ pour les créateurs de cours",
  },
  sections: {
    hero: "Hero",
    theProblem: "Le Problème",
    howItWorks: "Comment ça marche",
    tryItNow: "Essayez maintenant",
    examples: "Exemples",
    testimonials: "Témoignages",
    pricing: "Tarifs",
    getStarted: "Commencer",
  },
  modal: {
    closePreview: "Fermer l'aperçu",
  },
  toast: {
    paymentSuccess: "Paiement réussi ! Vous avez maintenant accès à Pro.",
    checkoutCancelled: "Paiement annulé. Vous pouvez réessayer à tout moment.",
    signInFailed: "Échec de la connexion. Veuillez réessayer.",
    courseGenerated: "Cours généré avec succès !",
  },
  contact: {
    backToHome: "Retour à l'accueil",
    badge: "Nous serions ravis de vous entendre",
    heading: "Prenez Contact",
    subheading:
      "Questions, idées ou juste envie de dire bonjour ? Nous construisons ensemble l'avenir de la création de cours.",
    emailTitle: "Écrivez-nous",
    emailDesc: "hello@syllabi.online",
    responseTitle: "Réponse rapide",
    responseDesc: "Généralement sous 24 heures",
    communityTitle: "Rejoindre la communauté",
    communityDesc: "Connectez-vous avec d'autres créateurs",
    nameLabel: "Votre nom",
    namePlaceholder: "Jean Dupont",
    emailLabel: "Adresse e-mail",
    emailPlaceholder: "vous@exemple.com",
    subjectLabel: "Sujet",
    subjectPlaceholder: "Comment pouvons-nous vous aider ?",
    messageLabel: "Message",
    messagePlaceholder: "Dites-nous ce que vous avez en tête...",
    sendBtn: "Envoyer le message",
    sendingBtn: "Envoi en cours...",
    sentTitle: "Message envoyé !",
    sentDesc: "Merci de nous avoir contactés. Nous vous répondrons bientôt.",
    sendAnother: "Envoyer un autre message",
    footerNote: "Vous pouvez aussi nous écrire directement à",
  },
  tutorial: {
    backToHome: "Retour à l'accueil",
    badge: "Guide étape par étape",
    heading: "Comment créer votre premier cours",
    subheading: "De l'idée au lien partageable en moins de 60 secondes.",
    step1Title: "Choisissez un modèle",
    step1Desc:
      "Commencez avec un modèle adapté à votre cas d'usage ou créez un cours entièrement personnalisé. Les modèles pré-remplissent le formulaire pour générer plus vite.",
    step2Title: "Décrivez votre cours",
    step2Desc:
      "Renseignez votre sujet, votre public et le style souhaité. Plus vous êtes précis, meilleur sera le résultat.",
    step3Title: "Générez et relisez",
    step3Desc:
      "Cliquez sur Générer. En quelques secondes, vous aurez un cours complet avec modules, leçons, quiz et narration audio.",
    step4Title: "Modifiez et personnalisez",
    step4Desc:
      "Utilisez l'éditeur de cours pour ajuster n'importe quelle section — titres, contenu, questions de quiz. Tout est modifiable.",
    step5Title: "Partagez ou exportez",
    step5Desc:
      "Copiez votre lien partageable, exportez en PDF ou vers Notion, ou téléchargez en fichier PPTX ou DOCX.",
    tipsTitle: "Conseils Pro",
    tip1: "Ajoutez une niche (ex. : « vente SaaS ») pour un contenu plus ciblé.",
    tip2: "Utilisez la fonctionnalité audio sur Pro Max pour créer des leçons au format podcast.",
    tip3: "Transformez les mini-cours en lead magnets en ajoutant une barrière e-mail.",
    cta: "Prêt à créer votre premier cours ?",
    ctaBtn: "Commencer gratuitement",
  },
  subpageNav: {
    backToHome: "Retour à l'accueil",
  },
};

export default fr;
