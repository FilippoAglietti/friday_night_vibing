...
class PDFBuilder {
  ...
  constructor(style: TeachingStyle = "conversational", creatorName: string = "Author") {
  ...
  private drawFooter(): void {
    ...
    this.doc.text(`By ${this.creatorName}`, PAGE.ml, fy);
  ...
  private buildCoverFormal(c: Curriculum): void {
    ...
    this.doc.text(`Published by ${this.creatorName}`, PAGE.width / 2, PAGE.height - 18, { align: "center" });
  }
  ...
  private buildCoverEditorial(c: Curriculum): void {
    ...
    // Footer
    this.font(F.xxs, "italic", C.textSec);
    this.doc.text(`By ${this.creatorName}`, PAGE.width / 2, PAGE.height - 14, { align: "center" });
  }
  ...
  private buildCoverWorkshop(c: Curriculum): void {
    ...
    this.doc.text(`Workshop pack by ${this.creatorName}`, PAGE.width / 2, PAGE.height - 6, { align: "center" });
  }
...
export function generateCurriculumPDF(
  ...
  try {
    const builder = new PDFBuilder(style ?? "conversational", creatorName);
